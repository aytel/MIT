...
\begin{enumerate}
  ...
  \item ~\\
	l) $\log(n^{\log n}) = \log n * \log \log n < \log^{2} n < n = \log_{2}1.1 \cdot \log (1.1^{n}) \to $ True. \\
 ...
 f(n) - o(f(n)) \ge f(n) - \frac {f(n)}{C} =  \frac {1 - C}{C} f(n) \to \frac {C}{1 - C}(f(n) - o(f(n)) \ge f(n) \to f(n) - o(f(n)) = \Omega(f(n)) \to $ True. Для $f(n) + o(f(n))$ аналогично.\\
  ...
  \item ~\\
	...
	Если без обобщённой мастер-теоремы, то можно так: \\
	$\Theta(n^2) = 4T(\frac{n}{2}) + n < 4T(\frac{n}{2}) + n\log^2n \to T(n) = \Omega(n^2)$. Докажем $O$. \\
	$T(n) < 4T(\frac{n}{2}) + n^{\frac{3}{2}} = \Theta(n^2)$, (последнее равенство -- по обычной мастер-теореме).
	$\to T(n) = O(n^2)$, \#. \\
...
\begin{enumerate}
  ...
  \setlength{\itemsep}{0pt} % Отступ между строками списка
  \item ~\\
	q) False. Докажем о/п. Пусть $\exists C : \forall n > N n^n \le Cn!$. Подставим в качестве $n X = max([C] + 1, N)$. \\
	$X^X < CX!$, поделим на $X$ слева и на $C$ справа. $X > C \to$ если после этого левая часть будет $\ge$ правой, то до такого деления она была $>$. \\
	$X^{X - 1} < X!$, в правой части можно убрать из произведения $1$, тогда останется $X - 1$ множителей $\le X$, а слева $X - 1$ множителей $X$. Значит, левая часть $>$ правой. Противоречие. \# \\
  \item ~\\
	l) Докажем, что $\forall  T(n) = 2F_{n} - 1$, где $F_{n}$ -- $n$-ое число Фибоначчи. Очевидно, для $n = 0, n = 1$ выполняется ($1 = 2 \cdot  1 - 1$), теперь переход. \\
	$T(n) = T(n - 1) + T(n - 2) + 1 = 2F_{n - 1} - 1 + 2F_{n - 2} -1 + 1 = 2F_{n} - 1 \to$ True. Тогда $T(n) = \Theta((\frac{\sqrt{5}+1}{2})^n)$. \\
	m) По теореме об эксп. рек. соотн. $T(n) = \Theta(\alpha^n)$, где $\alpha$ -- корень уравнения $1 = \alpha^{-1} + \alpha^{-3} \Leftrightarrow \alpha^{3} - \alpha^{2} - 1 = 0$. Можно взять формулы Кардано, можно бинпоиск на отрезке $[1, +\infty)$, ответ будет $\approx1.4656$.
